...
const { buildTaskLaunch } = require('./cli-launcher.cjs');
...
function buildCommand(agent, task) {
  ...
  const launch = buildTaskLaunch(agent, task, cwd);
  if (launch) return launch;

  const agentCommand = defaultCommandForAgent(agent);
  if (!agentCommand.trim()) return buildFallbackCommand(agent, task);
  const env = {
