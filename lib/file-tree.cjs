...
function resolveInsideWorkspace(workingDirectory, filePath) {
  if (!workingDirectory || !filePath) {
    throw new Error('Missing workspace or file path');
  }
...
function readWorkspaceFile(workingDirectory, filePath) {
  const targetPath = resolveInsideWorkspace(workingDirectory, filePath);
...
function writeWorkspaceFile(workingDirectory, filePath, content) {
  const targetPath = resolveInsideWorkspace(workingDirectory, filePath);
  const stats = fs.statSync(targetPath);
  ...
  return readWorkspaceFile(workingDirectory, targetPath);
}
